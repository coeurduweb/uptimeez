...
(`http` vers `https`, ajout de `www`), Uptimer le signale et propose d'aligner la sonde sur la cible en un clic.
...
même IP, Uptimer envoie **une** alerte groupée en disant que le problème est très probablement l'hébergement.
...
**Messages de console reconstitués.** Uptimer réécrit ce que le navigateur aurait affiché :
...
**Battement dead-man.** Votre sauvegarde, votre export, votre tâche nocturne appelle Uptimer quand elle a fini.
...
## Ce qu'Uptimer ne surveille volontairement pas

Un outil qui prétend tout faire ne fait rien de bien. Ce qui est hors de son champ, et pourquoi :

- **Les ports TCP, le ping ICMP, le DNS, le SMTP.** Uptimer surveille en HTTP. Un port ouvert ne dit pas qu'un
...
- **L'audit SEO complet.** Uptimer signale un `noindex` oublié parce que c'est un accident d'exploitation, pas
  parce qu'il fait de l'audit.

Uptimer fait une chose : **garder en vie les sites des autres, sans qu'un humain surveille un tableau de bord à
plein temps.**
