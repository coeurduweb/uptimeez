...
**Uptimer already knows which version runs on every site it watches. All that was left was to ask whether that
...
It rests on the same groundwork as broken-layout detection: Uptimer already fetches the HTML of every page it
...
"not up to date" gets ignored within three weeks, and the day it is right, nobody is looking. So Uptimer uses two
...
Worth stating plainly, because it is the only outbound traffic from Uptimer that does not go to a site you
...
- **It does not scan your site.** No request is sent to test a vulnerability, no admin path is probed, no
  payload is injected. Uptimer reads what the page publishes and queries public databases. A tool that actually
